<template>
  <div class="h-screen w-screen flex items-center justify-center bg-gradient-to-br from-green-800 to-green-600">
    <div class="w-full max-w-md bg-white rounded-xl shadow-xl overflow-hidden relative mx-4">
      ...
      <div class="absolute -top-10 -right-10 w-20 h-20 rounded-full bg-yellow-400 opacity-50"></div>
      
      <!-- Header with logo -->
      <div class="pt-8 pb-6 text-center">
        <div class="inline-flex items-center justify-center mb-4">
          <div class="relative w-16 h-16 bg-yellow-400 rounded-full flex items-center justify-center">
            <div class="absolute w-16 h-16 rounded-full border-2 border-white"></div>
            <div class="absolute w-14 h-14 rounded-full border-2 border-white"></div>
            <div class="absolute w-full h-0.5 bg-white"></div>
            <div class="absolute w-0.5 h-full bg-white"></div>
          </div>
        </div>
        <h1 class="text-2xl font-bold text-gray-800 mb-1">Tennis Community</h1>
      ...
      <div class="px-8 py-6 space-y-6">
        <!-- Error Alert -->
        <!-- Alert Error yang sesuai dengan screenshot terbaru -->
        <div v-if="loginError" class="bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-md relative mb-6">
          <div class="flex items-center justify-between">
            <p>Email atau password yang Anda masukkan salah.</p>
            <button @click="loginError = false" class="text-red-500 hover:text-red-700 ml-2">
        ...
        <form @submit.prevent="handleLogin" class="space-y-6 text-black">
          <div>
            <label for="email" class="block text-base font-medium text-gray-800 mb-2">Email</label>
            <input 
              type="email" 
              id="email" 
              v-model="email" 
              class="w-full px-4 py-3 bg-blue-50 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500"
          ...
          <div class="flex items-center space-x-3">
            <input type="checkbox" id="remember" class="h-5 w-5 text-green-600 focus:ring-green-500 border-gray-300 rounded" />
            <label for="remember" class="ml-2 block text-base text-gray-700 !ml-2">Remember me</label>
          </div>
          
          <button 
            type="submit" 
            class="w-full bg-green-500 text-white py-3 px-4 rounded-md hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-colors text-lg font-medium flex justify-center items-center"
            :disabled="isLoading"
          >
        ...
        <div class="mt-6 text-center">
          <p class="text-gray-600">
            Belum punya akun? <router-link to="/register" class="font-medium text-green-600 hover:text-green-800">Daftar</router-link>
          </p>
        </div>
        ...
        <div class="mt-8 space-y-6">
          ...
          <div class="mt-6 grid grid-cols-2 gap-4">
            <button type="button" class="flex justify-center items-center py-3 px-4 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-gray-700 hover:bg-gray-50">
      ...
      <div class="h-2 bg-green-600 mt-6"></div>
    </div>
  </div>
</template>
